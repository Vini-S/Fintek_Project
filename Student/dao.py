...
class Student:
	...
	def addleave(self, emailid, s_date, e_date, reason):
		cursor = connection.cursor()
		query = "insert into schedule_student_leave(s_email, s_date, e_date, l_reason) values (%s,%s,%s,%s)"
		values = (emailid, s_date, e_date, reason)
		cursor.execute(query,values)
		connection.commit()
		return 1

	def viewsleavebysession(self, emailid):       #(Auto fill form)
		cursor = connection.cursor()
		query = "select s_Email_id from schedule_student_view where s_Email_id=%s"
		values = (emailid)
		cursor.execute(query,values)
		row = cursor.fetchall()
		return row
...
class Faculty:
	...
	def fviewleave(self):
		cursor = connection.cursor()
		query = "select f_email, s_date, e_date from schedule_faculty_leave"
		cursor.execute(query)
		row = cursor.fetchall()
		return row
